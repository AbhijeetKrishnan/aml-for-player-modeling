...
class PddlProblem:
    ...
    def __init__(self, level):
        ...
        grid = []
        lines = level.split('\n')[:-1]  # The newline at the end creates an empty line
        for row in lines:
            grid.append([row[i:i + 1] for i in range(0, len(row), 1)])
        width, height = len(grid[0]), len(grid) - 1
        self.grid = grid
        ...
        for j in range(height):
            for i in range(width):
                ...
                for del_j, del_i, dir in [(1, 0, 'dir-down'), (0, -1, 'dir-left'), (0, 1, 'dir-right'),
                                          (-1, 0, 'dir-up')]:
