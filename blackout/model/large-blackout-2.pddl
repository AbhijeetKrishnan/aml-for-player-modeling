...
:precondition (and (clear ?4location)
)
:effect (and (clear ?2location) (at ?1stone ?4location) (at ?0player ?3location)
(not (at-goal ?1stone)) (not (at ?0player ?2location)) (not (at ?1stone ?3location)) (not (clear ?4location))))
(:action push-to-goal
:parameters (?0player - player ?1stone - stone ?2location - location ?3location - location ?4location - location ?5direction - direction)
:precondition (and (clear ?4location)
)
:effect (and (at-goal ?1stone) (at ?0player ?3location) (clear ?2location) (at ?1stone ?4location)
(not (at ?0player ?2location)) (not (at ?1stone ?3location)) (not (clear ?4location))))
)
